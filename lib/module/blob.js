...
function nitroFetchBlob() {
  ...
  try {
    ...
    _tm = NativeModules?.NitroFetchBlob ?? require('./turbomodule/NativeNitroFetchBlob').default ?? null;
  } catch {
    _tm = null;
  }
  return _tm ?? null;
...
export function bytesToBlob(bytes, type) {
  if (bytes.byteLength === 0) return new Blob([], {
    type
  });
  try {
    const bm = blobManager();
    const tm = nitroFetchBlob();
    if (typeof bm?.createFromOptions === 'function' && typeof tm?.createBlobId === 'function') {
  ...
  return new Blob([utf8ToString(new Uint8Array(bytes))], {
    type
  });
}
